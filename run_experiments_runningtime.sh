...
shopt -s nullglob
BASEDIR=.
CONFIGDIR=configs/esn_runningtime
...
ESNCONFIGDIR=${BASEDIR}/configs/esn_runningtime
...
for DATANAME in ${DATANAMES[@]}; do
  for CONFIGNAME in ${CONFIGNAMES[@]}; do
    ...
    ESNCONFIG=$ESNCONFIGDIR/${CONFIGNAME}
    ...
    unbuffer ./run_single_experiment_runningtime.sh $DATAFILE $CONFIGFILE $ESNCONFIG $RECONSTRUCTFILE $RUNS 2>&1 | tee $RESULTDIR/RunningTime/${DATANAME}_${CONFIGNAME}
